...
class Api {
  ...
  _getResponseData(res, errorPlace) {
    return res.ok ? res.json() : Promise.reject(`${this._errorMessages[errorPlace]} ${res.status}`);
  }
  ...
  _request(endPoint, options, errorPlace) {
    return fetch(`${this._baseURL}${endPoint}`, options).then(res => this._getResponseData(res, errorPlace));
  }
   ...
   */
  getUserData() {
    return this._request('/users/me', {headers: this._headers}, 'getUserData');
  }
